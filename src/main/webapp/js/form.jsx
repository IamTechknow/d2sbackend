...
const MAIN = 0; const SKILLS = 1; const ITEMS = 2; const MERC = 3; const VALID = 0; const MAX_LVL = 99; const
  MAX_SKILL_LVL = 20;
...
export default class Form extends Component {
    ...
    constructor(props) {
      ...
      const rewards = {};
      for (const key of ['den', 'imbue', 'skillBook', 'potion', 'lamEsen', 'izual', 'socket', 'scroll', 'nAncients', 'nmAncients', 'hAncients']) rewards[key] = false;
    ...
    getTimesCompleted(diff, quest, startingAct, act) {
      const timesBeatGame = Number.parseInt(diff) / 5;
      let timesCompleted = timesBeatGame * (quest ? 1 : 0);
      if (Number.parseInt(startingAct) >= act && quest) timesCompleted++;
    ...
    checkSkills() {
      const INVALID_NO_SP = 1; const INVALID_LEVEL_LOW = 2; const
        INVALID_DEPS = 3;
      const spLeft = this.calcSP() - this.state.allocated.reduce((accum, curr) => accum + curr);
      if (spLeft < 0) return INVALID_NO_SP;

      const skills = ClassData[this.state.classNum].skills;


      const offset = [6, 36, 66, 96, 126, 221, 251][this.state.classNum];

      for (let i = 0; i < this.state.allocated.length; i++) {
        if (this.state.allocated[i] > 0) {
          ...
          const deps = skills[i].deps;
          for (let j = 0; j < deps.length; j++) if (this.state.allocated[deps[j] - offset] < 1) return INVALID_DEPS;
        }
      }
      return VALID;
    }
    ...
    checkStats() {
      const INVALID_NEG_ATTR = 1; const
        STAT_BELOW_BASE = 2;
      const attr_left = this.calcStats() - this.state.attr.reduce((accum, curr) => accum + curr);
      if (attr_left < 0) return INVALID_NEG_ATTR;
    ...
    calcStats() { // Total number of stat points to spend
      const ACT3 = 2; const
        lamEsen = this.getTimesCompleted(this.state.difficulty, this.state.rewards.lamEsen, this.state.startingAct, ACT3);
      return 5 * (this.state.level - 1 + lamEsen);
    }

    calcSP() {
      const ACT2 = 1; const
        timesReadSkillBook = this.getTimesCompleted(this.state.difficulty, this.state.rewards.skillBook, this.state.startingAct, ACT2);
    ...
    onStatClick(event) {
      const arr = this.state.attr.slice();

      if (event.target.dataset.mode === '1') {
        const attr_left = this.calcStats() - arr.reduce((accum, curr) => accum + curr);
        if (attr_left > 0) arr[event.target.dataset.idx] += attr_left;
      } else {
    ...
    onFormChange(event) {
      const name = event.target.name, val = event.target.value;
      if (event.target.classList[0] === 'rewards') {
        const rewards = Object.assign({}, this.state.rewards);
        rewards[name] = event.target.checked;
        this.setState({ rewards });
      } else if (event.target.classList[0] === 'opts') {
        this.setState({ [name]: event.target.checked });
      } else if (event.target.classList[0] === 'skill') {
        const arr = this.state.allocated.slice();
        const idx = Number.parseInt(name.substring(6), 10);
        arr[idx] = Math.min(MAX_SKILL_LVL, Number.parseInt(val, 10));
        this.setState({ allocated: arr });
      } else if (name === 'level') {
        this.setState({ [name]: Math.min(MAX_LVL, Number.parseInt(val, 10)).toString() });
        ...
        const map = {
          ...
          idx = map[name];
        arr[idx] = Number.parseInt(val, 10) - ClassData[this.state.classNum].attributes[idx];
        this.setState({ attr: arr });
      } else {
        this.setState({ [name]: typeof val === 'number' ? Number.parseInt(val, 10) : val });
      }
    }
    ...
    handleSubmit(event) {
      ...
      const name = data.get('name'); const
      ...
      const invalidForClassic = parseInt(data.get('classNum')) >= 5 && !expansion;


      const invalidAct = parseInt(data.get('startingAct')) > 3 && !expansion;


      const invalidAncients = (nAncients && level < 20) || (nmAncients && level < 40) || (hAncients && level < 60);
      ...
      const invalid = invalidName || invalidForClassic || invalidAct || invalidAncients || invalidSkills > 0 || invalidStats > 0;
      this.setState({
        invalid,
        invalidName,
        invalidForClassic,
      ...
      for (var key in this.state.rewards) data.set(`rewards.${key}`, this.state.rewards[key]);

      for (var i = 0; i < this.state.allocated.length; i++) data.set(`skills[${i}]`, this.state.allocated[i]);
      ...
      if (this.state.valid) {
        return (
          <div className="container">
            <h1>Save created!</h1>
            <a href={this.state.link}>Download this save</a>
            {' '}
            <br />
            <a href="/">Create another save</a>
          </div>
        );
      } if (this.state.link = '' && !this.state.valid) {
        return (
          <div className="container">
            ...
            <form onSubmit={this.handleSubmit} noValidate>
              {this.state.currTab === MAIN
                            ...
                            <React.Fragment>
                              ...
                              <h4>Difficulty</h4>
                              <Difficulties currDiff={this.state.difficulty} handler={this.onFormChange.bind(this)} />

                              <h4>Starting Act</h4>
                              <Acts currAct={this.state.startingAct} handler={this.onFormChange.bind(this)} />
